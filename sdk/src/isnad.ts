...
// NOTE: IsnadRegistryContract is generated by `aztec codegen` after compilation.
// Import will work once contract is compiled and artifacts generated.
...
export class IsnadSDK {
  private constructor(
    ...
    // private readonly contract: IsnadRegistryContract  // enabled after codegen
  ) {}
  ...
  static async connect(
    ...
    return new IsnadSDK(wallet, contractAddress);
  }
  ...
  async getTrustScore(skillHash: string | Fr): Promise<SkillTrustInfo> {
    ...
    // TODO: enable after codegen
    // const [trustScore, attestationCount] = await Promise.all([
    //   this.contract.methods.get_trust_score(hash).simulate(),
    //   this.contract.methods.get_attestation_count(hash).simulate(),
    // ]);

    // Placeholder until contract is compiled
    const trustScore = 0n;
    const attestationCount = 0n;
  ...
  async attest(opts: AttestOptions): Promise<{ txHash: string }> {
    ...
    throw new Error("Contract not yet compiled — run `aztec compile` then `aztec codegen`");
  }
  ...
  async getCredential(keyId: string): Promise<CredentialResult | null> {
    ...
    //   .get_credential(ownerAddress, keyIdHash)
    //   .simulate();
    // if (result.is_none()) return null;
    // return { keyId, value: this._decodeValue(result.unwrap()), label: '' };
  ...
  async getCredentialForSkill(opts: DelegatedCredentialOptions): Promise<CredentialResult | null> {
    ...
    // TODO: enable after aztec codegen generates TypeScript bindings
    // const result = await this.contract.methods
    //   .get_credential_for_skill(opts.owner, keyId, new Fr(nonce))
    //   .send()
    //   .wait();
    // const rawValue = result.returnValue as [Fr, Fr, Fr, Fr];
    // return { keyId: opts.keyId, value: this._decodeValue(rawValue), label: '' };

    throw new Error("Contract not yet transpiled — run `aztec compile` then `aztec codegen`");
  }
  ...
  async grantCredentialAccess(opts: GrantAccessOptions): Promise<{ authwitNonce: bigint }> {
    ...
    // TODO: enable after aztec codegen generates TypeScript bindings
    // const action = this.contract.methods.get_credential_for_skill(ownerAddress, keyId, new Fr(nonce));

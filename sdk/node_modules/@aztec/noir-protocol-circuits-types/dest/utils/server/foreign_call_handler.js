import { BatchedBlobAccumulator, BlobAccumulator, FinalBlobBatchingChallenges, computeBlobFieldsHash, getBlobsPerL1Block } from '@aztec/blob-lib';
import { BLOB_ACCUMULATOR_LENGTH, BLS12_FQ_LIMBS, BLS12_FR_LIMBS, BLS12_POINT_LENGTH, FIELDS_PER_BLOB } from '@aztec/constants';
import { chunk } from '@aztec/foundation/collection';
import { BLS12Fq, BLS12Fr, BLS12Point } from '@aztec/foundation/curves/bls12';
import { Fr } from '@aztec/foundation/curves/bn254';
import { LogLevels, applyStringFormatting, createLogger } from '@aztec/foundation/log';
import { strict as assert } from 'assert';
export async function foreignCallHandler(name, args) {
    ...
    if (name === 'utilityLog') {
        ...
        const paddedBlobsAsFr = flattenedArgs.slice(offset, offset += FIELDS_PER_BLOB * numBlobs).map((field)=>Fr.fromString(field));
        ...
        const kzgCommitmentsFields = chunk(flattenedArgs.slice(offset, offset += BLS12_POINT_LENGTH * numBlobs), BLS12_POINT_LENGTH);
        const kzgCommitments = kzgCommitmentsFields.map((fields)=>{
            const x = BLS12Fq.fromNoirBigNum({
                limbs: fields.slice(0, BLS12_FQ_LIMBS)
            });
        ...
        // TODO(#14646): Omit/compress some fields to reduce number of public inputs & outputs here?
        const finalBlobChallenges = new FinalBlobBatchingChallenges(Fr.fromString(flattenedArgs[offset++]), BLS12Fr.fromNoirBigNum({
            limbs: flattenedArgs.slice(offset, offset += BLS12_FR_LIMBS)
        }));
        ...
        if (!expectedBlobFieldsHash.equals(blobFieldsHash)) {
            throw new Error(`Injected blob fields do not match rolled up fields. Computed hash: ${blobFieldsHash}. Hash used in circuits: ${expectedBlobFieldsHash}.`);
        }
        ...
        const batchedBlobAccumulator = new BatchedBlobAccumulator(startBlobAccumulator.blobCommitmentsHashAcc, startBlobAccumulator.zAcc, startBlobAccumulator.yAcc, startBlobAccumulator.cAcc, BLS12Point.ZERO, startBlobAccumulator.gammaAcc, startBlobAccumulator.gammaPowAcc, finalBlobChallenges);
        ...
        return Promise.resolve([
            endBlobAccumulator.toFields().map((field)=>field.toString())
        ]);
    } else if (name === 'noOp') {

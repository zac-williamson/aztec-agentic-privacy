...
async function buildVKTree() {
    const calculator = await MerkleTreeCalculator.create(VK_TREE_HEIGHT, Buffer.alloc(32), async (a, b)=>(await poseidon2Hash([
            a,
            b
        ])).toBuffer());
    const vkHashes = new Array(2 ** VK_TREE_HEIGHT).fill(Buffer.alloc(32));
    for (const [key, value] of Object.entries(allVks)){
        const index = ProtocolCircuitVkIndexes[key];
...
export const vkTree = new MerkleTree(${vkTree.height}, [${vkTree.nodes.map((node)=>`'${node.toString('hex')}'`).join(', ')}

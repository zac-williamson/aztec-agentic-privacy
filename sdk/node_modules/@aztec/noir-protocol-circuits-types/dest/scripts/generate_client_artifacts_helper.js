...
const ClientCircuitArtifactNames = {
...
function generateSimulatedArtifactName(artifactName) {
    const isReset = artifactName.includes('private_kernel_reset');
    return isReset ? artifactName.replace('private_kernel_reset', 'private_kernel_reset_simulated') : `${artifactName}_simulated`;
}
function generateCircuitArtifactImportFunction() {
    const cases = Object.values(ClientCircuitArtifactNames).flatMap((artifactName)=>{
        const hasSimulatedVersion = !artifactsWithoutSimulatedVersions.includes(artifactName);
        return hasSimulatedVersion ? [
            artifactName,
            generateSimulatedArtifactName(artifactName)
        ] : [
            artifactName
        ];
    }).map((artifactName)=>{

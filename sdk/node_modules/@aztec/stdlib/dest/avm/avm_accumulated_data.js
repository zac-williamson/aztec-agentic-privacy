...
export class AvmAccumulatedData {
   ...
   */ static fromPlainObject(obj) {
        return new AvmAccumulatedData(assertLength(obj.noteHashes.map((h)=>Fr.fromPlainObject(h)), MAX_NOTE_HASHES_PER_TX), assertLength(obj.nullifiers.map((n)=>Fr.fromPlainObject(n)), MAX_NULLIFIERS_PER_TX), assertLength(obj.l2ToL1Msgs.map((m)=>ScopedL2ToL1Message.fromPlainObject(m)), MAX_L2_TO_L1_MSGS_PER_TX), FlatPublicLogs.fromPlainObject(obj.publicLogs), assertLength(obj.publicDataWrites.map((w)=>PublicDataWrite.fromPlainObject(w)), MAX_TOTAL_PUBLIC_DATA_UPDATE_REQUESTS_PER_TX));
    }
    isEmpty() {

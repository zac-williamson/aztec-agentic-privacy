...
export class DelayedPublicMutableValues {
    ...
    static async readFromTree(delayedPublicMutableSlot, readStorage) {
        const fieldPromises = Array.from({
            length: DELAYED_PUBLIC_MUTABLE_VALUES_LEN
        }).map((_, i)=>readStorage(delayedPublicMutableSlot.add(new Fr(i))));
        return DelayedPublicMutableValues.fromFields(await Promise.all(fieldPromises));
    }
    isEmpty() {
        return this.svc.isEmpty() && this.sdc.isEmpty();
    }
    writeToTree(delayedPublicMutableSlot, storageWrite) {
        const fieldPromises = this.toFields().map((field, i)=>storageWrite(delayedPublicMutableSlot.add(new Fr(i)), field));

...
export async function createPXE(aztecNode, config, options = {
    ...
    const recorderLogger = createLogger('simulator:acvm:recording', {
        actor
    });
    const recorder = process.env.CIRCUIT_RECORD_DIR ? new FileCircuitRecorder(process.env.CIRCUIT_RECORD_DIR, recorderLogger) : new MemoryCircuitRecorder(recorderLogger);
    const simulatorLogger = createLogger('wasm-simulator', {
        actor
    });
    ...
    if (!options.store) {
        const storeLogger = loggers.store ?? createLogger('pxe:data:lmdb', {
            actor
        });
        options.store = await createStore('pxe_data', PXE_DATA_SCHEMA_VERSION, configWithContracts, storeLogger.getBindings());
    }
    const proverLogger = loggers.prover ?? createLogger('pxe:bb:native', {
        actor
    });
    let prover;
